...
void FIRCalcHannWindow(int length, double *window)
{
  if ((length % 2) == 0)
  {
    for (int i=0; i<length; i++)
    {
      ...
      window[i] = 0.5 + 0.5*cos(2*PI*n/(length-1));
    }
  }
  else 
  {
...
void FIRCalcHammingWindow(int length, double *window)
{
  if ((length % 2) == 0)
  {
    ...
    {
      double n = i - (length / 2.0) + 0.5;
      window[i] = (25.0/46.0) + (21.0/46.0)*cos(2*PI*n/(length-1));
    }
  }
  else 
  {
  ...
  if ((length % 2) == 0)
  {
    for (int i=0; i<length; i++)
    {
      ...
      window[i] = (1-alpha)/2.0 -
        (1/2)*cos(2*PI*n/(length-1)) +
        (alpha/2.0)*cos(4*PI*n/(length-1));
    }
  }
  else 
  {
...
void FIRCalcKaiserWindow(int length, double *window, double alpha)
{
  ...
  if ((length % 2) == 0)
  {
    ...
    {
      double n = i - (length / 2.0) + 0.5;
      window[i] = FIRCalcBessi0(PI*alpha*sqrt(1-pow((2*n/(length-1)),2))) / denom;
    }
  }
  else 
  {
